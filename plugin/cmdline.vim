...
augroup delay_cycles_set
    au!
    " TODO: previously we wrote a single autocmd:{{{
    "
    "     au CmdlineEnter : call s:cycles_set() | exe 'au! delay_cycles_set' | aug! delay_cycles_set
    "
    " Now, write `finish` at the top of:
    "
    "     ~/.vim/plugged/vim-cmdline/autoload/cmdline/cycle/vimgrep.vim
    "
    " And restart Vim.
    " Every time you enter the command-line, an error is raised.
    " This   augroup  was   not   correctly  removed,   because   the  call   to
    " `s:cycles_set()` raised an error.
    " So, we remove the augroup from another autocmd.
    "
    " Alternatively, we could prefix the call with `:silent!`.
    " But a second autocmd is better.
    " In other cases, `au!` may be prefixed by a whole pipeline of commands.
    " Prefixing each  of them with `:silent!`  would be brittle, and  maybe even
    " undesirable (if  the first  command fails,  maybe we  don't want  the next
    ...
    "
    " For the same  reason, you can't reliably clear an  autocmd from a function
    " in which other commands are present before (because of `abort`).
    "
    " Other MWE:
    "
    "     augroup test_sth
    ...
    " We probably made the same mistake in other plugins; fix it everywhere.
    "}}}
